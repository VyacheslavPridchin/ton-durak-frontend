  ...
  <div v-if="isVisible" class="popup-overlay" ref="popupContainer">
    <div class="popup">
      <div class="popup-header">
        ...
        <h2 class="popup-title">Ты уверен?</h2>
      </div>
      <p class="popup-subtitle">Ты потеряешь ${{ bid }}</p>
      <button class="continue-button" @click="hidePopup">Продолжить играть</button>
      <button class="leave-button" @click="leaveGame">Выйти из игры</button>
    </div>
  </div>
</template>
...
function leaveGame() {
  NetworkManager.surrendered = true;
  NetworkManager.CloseGame(router);
}

function onShowTryLeavePopup() {
  showPopup();
}

function onBidSet(amount: number) {
  bid.value = amount;
}

onMounted(() => {
  loadIcon();
  EventService.Instance.on(EventType.ShowTryLeavePopup, onShowTryLeavePopup);
  EventService.Instance.on(EventType.BidSet, onBidSet);
});

onUnmounted(() => {
  EventService.Instance.off(EventType.ShowTryLeavePopup, onShowTryLeavePopup);
