<template>
  <div class="container">
    <div class="page">
      <h2 style="margin-bottom: 1vh; margin-top: 12vh">Депозит</h2>
      <a class="secondary-text" style="margin-bottom: 1vh;">Выберите сеть депозита</a>
      <div class="list">
        <CryptoItem
            :crypto-network="'USDT TON'"
            :icon-src="USDTTonIcon"
            @click="() => {router.push('/deposit?network=USDT TON')}"
        />
        <CryptoItem
            :crypto-network="'TON'"
            :icon-src="TonIcon"
            @click="() => {router.push('/deposit?network=TON')}"
        />
      </div>
    </div>
  </div>
